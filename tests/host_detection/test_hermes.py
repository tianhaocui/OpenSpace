...
class TestResolveConfigPath:
    ...
    def test_home_based_config(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.delenv("HERMES_CONFIG_PATH", raising=False)
        monkeypatch.setenv("HERMES_HOME", "/opt/hermes")
        assert _resolve_config_path() == Path("/opt/hermes/config.yaml")
...
class TestIsHermesHost:
    ...
    def test_false_when_no_config(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setenv("HERMES_CONFIG_PATH", str(tmp_path / "nonexistent.yaml"))
...
class TestReadHermesMcpEnv:
    ...
    def test_reads_env_block(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        config.write_text(
            ...
            "  openspace:\n"
            "    env:\n"
            "      OPENSPACE_API_KEY: hermes-key-123\n"
            "      OPENSPACE_MODEL: gpt-4\n"
        )
        monkeypatch.setenv("HERMES_CONFIG_PATH", str(config))
        env = read_hermes_mcp_env("openspace")
        assert env == {"OPENSPACE_API_KEY": "hermes-key-123", "OPENSPACE_MODEL": "gpt-4"}

    def test_returns_empty_when_no_server(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        config = tmp_path / "config.yaml"
        config.write_text("mcp_servers:\n  other_server:\n    env: {}\n")
        monkeypatch.setenv("HERMES_CONFIG_PATH", str(config))
        assert read_hermes_mcp_env("openspace") == {}
...
class TestTryReadHermesConfig:
    ...
    def test_falls_back_to_env_for_api_key(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        config = tmp_path / "config.yaml"
        config.write_text("model:\n  provider: openai\n  default: gpt-4\n")
        monkeypatch.setenv("HERMES_CONFIG_PATH", str(config))
...
class TestGetHermesOpenaiApiKey:
    ...
    def test_falls_back_to_mcp_env(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        config.write_text(
            ...
            "  openspace:\n"
            "    env:\n"
            "      OPENAI_API_KEY: sk-from-hermes\n"
        )
        monkeypatch.setenv("HERMES_CONFIG_PATH", str(config))
        assert get_hermes_openai_api_key() == "sk-from-hermes"
